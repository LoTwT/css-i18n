...
import {
  highlightLinePlugin,
  preWrapperPlugin,
} from "./markdown-it/highlightLines"
...
export async function setupMarkdownPlugins(
  ...
  if (!isExcerpt) md.use(headingPlugin, mdOptions?.toc?.includeLevel)
  ...
  if (mdOptions.config) mdOptions.config(md)
